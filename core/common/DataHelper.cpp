...
void assign(NTimestamp& time, const google::protobuf::Timestamp& data) { time = data.seconds() * 1000; }

void assign(bool& b, const ::google::protobuf::BoolValue& data) { b = data.value(); }

void assign(int32_t& value, const ::google::protobuf::Int32Value& data) { value = data.value(); }
...
void assign(NAccountDevice& device, const nakama::api::AccountDevice& data) { assign(device.id, data.id()); }
...
void assign(NUsers& users, const nakama::api::Users& data) { assign(users.users, data.users()); }
...
void assign(NMatchList& list, const nakama::api::MatchList& data) { assign(list.matches, data.matches()); }
...
void assign(NStatus& status, const ::nakama::realtime::Status& data) { assign(status.presences, data.presences()); }
