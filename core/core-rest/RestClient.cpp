...
void RestClient::disconnect() { _httpClient->cancelAllRequests(); }

void RestClient::tick() { _httpClient->tick(); }
...
void RestClient::setBasicAuth(RestReqContext* ctx) { ctx->auth.append(_basicAuthMetadata); }
      ...
      if (reqContext->successCallback) {
        ...
        if (reqContext->data) {
          ...
          if (!ok) {
            reqError(
                reqContext, NError(
                                "Parse JSON failed. HTTP body: " + response->body + " error: " + status.ToString(),
  ...
  _TRY_BEGIN
    ...
    if (successCallback) {
      ctx->successCallback = [sessionData, successCallback]() {
        NSessionPtr session(
            new DefaultSession(sessionData->token(), sessionData->refresh_token(), sessionData->created()));
...
void RestClient::updateAccount(
  ...
  _TRY_BEGIN
    ...
    if (username)
      document.AddMember("username", *username, document.GetAllocator());
    if (displayName)
      document.AddMember("display_name", *displayName, document.GetAllocator());
    if (avatarUrl)
      document.AddMember("avatar_url", *avatarUrl, document.GetAllocator());
    if (langTag)
      document.AddMember("lang_tag", *langTag, document.GetAllocator());
    if (location)
      document.AddMember("location", *location, document.GetAllocator());
    if (timezone)
      document.AddMember("timezone", *timezone, document.GetAllocator());
...
void RestClient::getUsers(
  ...
  _TRY_BEGIN
    ...
    for (auto& username : usernames) {
      args.emplace("usernames", encodeURIComponent(username));
    }
...
void RestClient::listFriends(
  ...
  _TRY_BEGIN
    ...
    if (state)
      args.emplace("state", std::to_string((int32_t)*state));
    if (!cursor.empty())
...
void RestClient::createGroup(
  ...
  _TRY_BEGIN
    ...
    if (maxCount)
      document.AddMember("max_count", *maxCount, document.GetAllocator());
...
void RestClient::writeLeaderboardRecord(
  ...
  _TRY_BEGIN
    ...
    if (subscore)
      document.AddMember("subscore", std::to_string(*subscore), document.GetAllocator());
    if (metadata)
      document.AddMember("metadata", *metadata, document.GetAllocator());
...
void RestClient::listMatches(
  ...
  _TRY_BEGIN
    ...
    if (min_size)
      args.emplace("min_size", std::to_string(*min_size));
    if (max_size)
      args.emplace("max_size", std::to_string(*max_size));
    if (limit)
      args.emplace("limit", std::to_string(*limit));
    if (label)
      args.emplace("label", encodeURIComponent(*label));
    if (query)
      args.emplace("query", encodeURIComponent(*query));
    if (authoritative)
...
void RestClient::deleteNotifications(
  ...
  _TRY_BEGIN
    ...
    if (successCallback) {
      ctx->successCallback = [successCallback]() { successCallback(); };
    }
    ctx->errorCallback = errorCallback;
...
void RestClient::listTournaments(
  ...
  _TRY_BEGIN
    ...
    if (categoryStart)
      args.emplace("category_start", std::to_string(*categoryStart));
    if (categoryEnd)
      args.emplace("category_end", std::to_string(*categoryEnd));
    if (startTime)
      args.emplace("start_time", std::to_string(*startTime));
    if (endTime)
      args.emplace("end_time", std::to_string(*endTime));
    if (limit)
      args.emplace("limit", std::to_string(*limit));
    if (cursor)
      args.emplace("cursor", encodeURIComponent(*cursor));
