...
public:
  ...
  void sendMatchData(
      ...
      std::int64_t opCode,
      const NBytes& data,
      const std::vector<NUserPresence>& presences = {}) override;
  ...
  writeChatMessageAsync(const std::string& channelId, const std::string& content) override;
  ...
  std::future<NMatch> joinMatchByTokenAsync(const std::string& token) override;
  ...
  std::future<NStatus> followUsersAsync(const std::vector<std::string>& userIds) override;

  std::future<void> unfollowUsersAsync(const std::vector<std::string>& userIds) override;
  ...
  std::future<NPartyMatchmakerTicket> addMatchmakerPartyAsync(
      const std::string& partyId,
      const std::string& query,
      int32_t minCount,
      int32_t maxCount,
