...
NRtClient::NRtClient(NRtTransportPtr transport, const std::string& host, int32_t port, bool ssl)
    : _host(host), _port(port), _ssl(ssl), _transport(transport), _connectPromise(nullptr) {
  NLOG_INFO("Created");
...
void NRtClient::setListener(NRtClientListenerInterface* listener) { _listener = listener; }
...
bool NRtClient::isConnecting() const { return _transport->isConnecting(); }

bool NRtClient::isConnected() const { return _transport->isConnected(); }
...
void NRtClient::joinChat(
  ...
  if (persistence)
    channelJoin->mutable_persistence()->set_value(*persistence);
  if (hidden)
    channelJoin->mutable_hidden()->set_value(*hidden);
...
void NRtClient::leaveChat(
  ...
  if (successCallback) {
    ctx->successCallback = [successCallback](::nakama::realtime::Envelope& /*msg*/) { successCallback(); };
  }
  ctx->errorCallback = errorCallback;
...
void NRtClient::writeChatMessage(
    const std::string& channelId,
    const std::string& content,
    std::function<void(const NChannelMessageAck&)> successCallback,
    RtErrorCallback errorCallback) {
  NLOG_INFO("...");
...
void NRtClient::updateChatMessage(
    const std::string& channelId,
...
void NRtClient::removeChatMessage(
    const std::string& channelId,
    const std::string& messageId,
    std::function<void(const NChannelMessageAck&)> successCallback,
    RtErrorCallback errorCallback) {
...
void NRtClient::joinMatchByToken(
...
void NRtClient::addMatchmaker(
  ...
  if (minCount)
    data->set_min_count(*minCount);
  if (maxCount)
    data->set_max_count(*maxCount);
  if (query)
    data->set_query(*query);
  if (countMultiple)
    data->mutable_count_multiple()->set_value(*countMultiple);
...
void NRtClient::removeMatchmaker(
...
void NRtClient::followUsers(
    const std::vector<std::string>& userIds,
    std::function<void(const NStatus&)> successCallback,
    RtErrorCallback errorCallback) {
  NLOG_INFO("...");
...
void NRtClient::addMatchmakerParty(
    const std::string& partyId,
...
void NRtClient::createParty(
    bool open,
    int maxSize,
    std::function<void(const NParty&)> successCallback,
    RtErrorCallback errorCallback) {
  NLOG_INFO("...");
...
void NRtClient::listPartyJoinRequests(
    const std::string& partyId,
    std::function<void(const NPartyJoinRequest&)> successCallback,
    RtErrorCallback errorCallback) {
  NLOG_INFO("...");
...
void NRtClient::promotePartyMember(
    const std::string& partyId,
...
void NRtClient::removeMatchmakerParty(
    const std::string& partyId,
    const std::string& ticket,
    std::function<void()> successCallback,
    RtErrorCallback errorCallback) {
...
void NRtClient::removePartyMember(
    const std::string& partyId,
    NUserPresence& presence,
    std::function<void()> successCallback,
    RtErrorCallback errorCallback) {
...
std::future<NChannelPtr> NRtClient::joinChatAsync(
  ...
  joinChat(
      target, type, persistence, hidden, [=](NChannelPtr channel) { promise->set_value(channel); },
...
std::future<NChannelMessageAck>
NRtClient::writeChatMessageAsync(const std::string& channelId, const std::string& content) {
  auto promise = std::make_shared<std::promise<NChannelMessageAck>>();

  writeChatMessage(
      channelId, content, [=](const NChannelMessageAck& ack) { promise->set_value(ack); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });
...
std::future<NChannelMessageAck> NRtClient::updateChatMessageAsync(
  ...
  updateChatMessage(
      channelId, messageId, content, [=](const NChannelMessageAck& ack) { promise->set_value(ack); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });

  return promise->get_future();
}

std::future<void> NRtClient::removeChatMessageAsync(const std::string& channelId, const std::string& messageId) {
...
std::future<NMatch> NRtClient::joinMatchAsync(const std::string& matchId, const NStringMap& metadata) {
  auto promise = std::make_shared<std::promise<NMatch>>();

  joinMatch(
      matchId, metadata, [=](const NMatch& match) { promise->set_value(match); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });

  return promise->get_future();
}

std::future<NMatch> NRtClient::joinMatchByTokenAsync(const std::string& token) {
  auto promise = std::make_shared<std::promise<NMatch>>();

  joinMatchByToken(
      token, [=](const NMatch& match) { promise->set_value(match); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });

  return promise->get_future();
}

std::future<void> NRtClient::leaveMatchAsync(const std::string& matchId) {
  leaveMatch(matchId);
  auto promise = std::make_shared<std::promise<void>>();
  promise->set_value();
  return promise->get_future();
...
std::future<NMatchmakerTicket> NRtClient::addMatchmakerAsync(
  ...
  addMatchmaker(
      minCount, maxCount, query, stringProperties, numericProperties, countMultiple,
      [=](const NMatchmakerTicket& ticket) { promise->set_value(ticket); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });

  return promise->get_future();
}

std::future<void> NRtClient::removeMatchmakerAsync(const std::string& ticket) {
...
std::future<NStatus> NRtClient::followUsersAsync(const std::vector<std::string>& userIds) {
  auto promise = std::make_shared<std::promise<NStatus>>();

  followUsers(
      userIds, [=](const NStatus& ticket) { promise->set_value(ticket); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });

  return promise->get_future();
}

std::future<void> NRtClient::unfollowUsersAsync(const std::vector<std::string>& userIds) {
...
std::future<NPartyMatchmakerTicket> NRtClient::addMatchmakerPartyAsync(
    const std::string& partyId,
    const std::string& query,
    int32_t minCount,
    int32_t maxCount,
  ...
  addMatchmakerParty(
      partyId, query, minCount, maxCount, stringProperties, numericProperties, countMultiple,
      [=](const NPartyMatchmakerTicket& ticket) { promise->set_value(ticket); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });
...
std::future<NParty> NRtClient::createPartyAsync(bool open, int maxSize) {
  ...
  createParty(
      open, maxSize, [=](const NParty& party) { promise->set_value(party); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });
...
std::future<NPartyJoinRequest> NRtClient::listPartyJoinRequestsAsync(const std::string& partyId) {
  ...
  listPartyJoinRequests(
      partyId, [=](const NPartyJoinRequest& partyJoinRequest) { promise->set_value(partyJoinRequest); },
      [=](const NRtError& error) { promise->set_exception(std::make_exception_ptr<NRtException>(error)); });
...
std::shared_ptr<RtRequestContext> NRtClient::createReqContext(::nakama::realtime::Envelope& msg) {
  ...
  while (!inserted && --numTries > 0) {
    ...
    if (!inserted) {
      NLOG(
          NLogLevel::Error, "Creating request with already assigned CID=%d, please report this bug to Heroic Labs",
          cid);
    }
  }
...
void NRtClient::heartbeat() {
  if (!_heartbeatIntervalMs.has_value()) {
    return;
  }
  ...
  if (_wantDisconnect || !_transport->isConnected()) {
    return;
  }
  ...
  {
    std::lock_guard<std::mutex> lock(_reqContextsLock);
    if (!_reqContexts.empty()) {
      return;
    }
  }

  // not yet
  if (_lastMessageTs + heartbeatIntervalMs > now) {
    return;
  }

  auto pingSuccess = [this]() { _lastHeartbeatTs = 0; };
